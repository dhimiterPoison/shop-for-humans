        ...
        const response = await new Promise<OrderDetails>((resolve) => {
          setTimeout(() => {
            resolve({
              ...
              status: 'paid',
              items: [
                { id: 'item1', name: 'T-Shirt', quantity: 2, price: 2999 },
                { id: 'item2', name: 'Jeans', quantity: 1, price: 9999 },
              ],
            })
          }, 1000)
        })
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              ...
              <p>Total Amount: ${(orderDetails.amount / 100).toFixed(2)}</p>
            </div>
            <div>
              ...
              <ul className="list-disc list-inside">
                {orderDetails.items.map((item) => (
                  <li key={item.id}>
                    {item.name} - Quantity: {item.quantity} - ${(item.price / 100).toFixed(2)} each
                  </li>
                ))}
              </ul>
            </div>
